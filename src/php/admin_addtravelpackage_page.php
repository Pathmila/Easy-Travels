...
<?php
	...
	if(isset($_POST['submit'])){
		...
		if(($_POST['destination1'] != 'null') || ($_POST['destination2'] != 'null') || ($_POST['destination3'] != 'null') || ($_POST['destination4'] != 'null') || ($_POST['destination5'] != 'null')){
			$_GLOBAL['destin']=1;
		}else{
			$_GLOBAL['destin']=0;
			echo "<script> alert('Insert destination before submit.') </script>";				
		}

		$check_name = check_name($connection, $name);
		if($check_name == 1){
			if(($_GLOBAL['destin']==1)){
				...
				if(($_GLOBAL['package']==1) && ($_GLOBAL['categorysubcategory']==1)){
