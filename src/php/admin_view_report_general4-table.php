...
<?php
	...
	while($row=$result3->fetch_assoc()){				
		echo "<tr>";
		echo "<td> <form method='post' action='admin_view_report_general4.php'><input name='viewticket' type='submit' id='next' value='".$row['cid']."' class='next'>";
		echo "<input type='hidden' name='selectedID' value='".$row['cid']."'/></form></td>";

		echo "<td>".$row['uid']."</td>";
		echo "<td>".$row['details']."</td>";
					
		$photo1=$row['photo1'];
		echo "<td><img class='image' width='140px' src='".$path.$photo1.$ex."'></td>";
		//$photo2=$row['photo2'];
		//echo "<td><img class='image' width='120px' src='".$path.$photo2.$ex."'></td>";
		//$photo3=$row['photo3'];
		//echo "<td><img class='image' width='120px' src='".$path.$photo3.$ex."'></td>";
